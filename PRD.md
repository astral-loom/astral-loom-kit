...
- [x] Each repo has a clear README with install + usage examples
- [x] Each repo has CONTRIBUTING.md with local setup instructions
- [x] Each repo has a LICENSE (MIT)
...
## 10. How to Use This PRD (Working with AI Coding Agents)

This document is the **north star** for the organization — it keeps scope, architecture, and roadmap consistent across sessions and across all three repos. It is intentionally high-level and is **not a substitute** for detailed build instructions when generating actual code.

### What this PRD is good for
- Giving an agent (or a new contributor) the overall vision and how the three repos relate to each other
- Keeping feature work aligned to the roadmap phase the org is currently in
- Referencing the Points/complexity model when scoping issues
- Preventing scope drift across long-running or multi-session agent work

### What this PRD does NOT replace
- **Feature-level build prompts.** A roadmap bullet like "batch payment helper" or "Albedo adapter" is not enough detail for an agent to implement well. Each new feature still needs a specific build prompt with function signatures, folder structure, libraries, and edge cases — the same level of detail as the original `astral-loom-kit` scaffold prompt.
- **Actual issue text.** The PRD says issues should be opened per repo, but doesn't contain the issue titles/descriptions/acceptance criteria themselves. Those should be drafted separately, based on the real code that exists at the time.
- **Operational/mechanical steps.** Git auth, GitHub org/repo settings, CI configuration, and Drips Wave onboarding clicks are procedural steps that need to be done directly — a PRD reference won't cause an agent to "figure out" credentials or GitHub UI flows.

### Recommended workflow going forward
1. Point the agent at this PRD for context/vision at the start of a session
2. Give a specific, detailed build prompt for whatever feature or repo scaffold is next
3. Once real code exists, draft issue text separately (don't rely on the agent to invent issue descriptions from the PRD alone)
4. Update the checkboxes in Section 7 (Roadmap) as phases complete, so the PRD stays accurate over time
